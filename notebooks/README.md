...
Jupyter 最好的地方，从来不只是“把代码放进浏览器”这么简单，而是它把讲解、代码、图像和结果放在同一页上，让你可以边看边试、边改边想。你写下的一行字、敲下的一格代码、画出来的一张图，都会立刻回到你面前。学习因此不再是远远地听，而是亲手地做。

这份教程也正是按这个思路设计的：我们不会一上来就把你推进黑箱里，而是从声音最朴素的数字表示开始，一路走到音频处理、深度学习、语音识别、音乐理解，以及 LLM + RAG 的核心链路。每一段内容都尽量先给你一个能看见的直觉，再给你一格能运行的代码，最后再把公式、算法和工程实现一层层对上去。你会发现，原来很多看起来很大的概念，拆开以后也不过是一些可以手写、可以调试、可以验证的小步骤。

如果你第一次打开 Jupyter，也不必着急把自己训练成“已经会了”的样子。Notebook 本来就是给试错、修正、再试一次准备的；`✏️ TODO` 不是陷阱，而是刻意留给你的空白，检查格也不是审判，而是帮你确认理解真的落了地。真正重要的，不是第一遍就写得漂亮，而是你愿意把每一步都跑出来、看明白、再改好。

所以，接下来我们会用最适合 notebook 的节奏一起往前走：一格一格地学，一步一步地做。先把声音变成数组，再把数组变成特征，把特征变成模型，最后把模型变成你真正能讲清楚、能修改、也能继续扩展的系统。路会长一点，但每走一步，你都会比前一刻更接近“原来它真是这么回事”的那种踏实感。

祝你在这门课里学得顺，学得稳，也学得开心。下面我们就按阶段，把这条线慢慢展开。
...
- 课程 bug 反馈：[dev@0xGarfield.com](mailto:dev@0xGarfield.com)（请附课程号、文件名、复现步骤和截图）
