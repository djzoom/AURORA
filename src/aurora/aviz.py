...
def style(theme: str | None = None):
    ...
    theme_spec = apply_theme(
        theme, cjk_font=find_cjk_font(), figure_size=(10, 4), font_size=12
    )
...
def aliasing(f_signal, f_sample):
    ...
    ax.plot(t_fine, y_true, "C0-", lw=2, alpha=0.55, label=f"原始信号 {f_signal} Hz")
    ax.plot(t_fine, y_alias, "C1--", lw=2, label=f"混叠频率 {f_alias} Hz")
    ...
    ax.set_xlabel("时间 (s)")
    ax.set_title(f"混叠：{f_signal} Hz × {f_sample} Hz 采样率 → 混叠为 {f_alias} Hz")
    plt.tight_layout()
    plt.show()
