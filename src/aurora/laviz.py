...
def _annotate_cells(ax, M, fontsize=9):
    ...
    for i in range(M.shape[0]):
        for j in range(M.shape[1]):
            ax.text(j, i, f"{M[i, j]:.2g}", ha="center", va="center", fontsize=fontsize)
...
def arrows2d(vectors, labels=None, colors=None, title=""):
    ...
    for vec, lbl, col in zip(vecs, labels, colors, strict=False):
        ax.annotate(
            ...
            xy=vec,
            xytext=(0, 0),
            arrowprops=dict(arrowstyle="->", color=col, lw=2, mutation_scale=18),
        )
        ax.text(
            vec[0] * 1.08,
            vec[1] * 1.08,
...
def vec_times_vec(a, b):
    ...
    plt.suptitle(
        "向量 × 向量 — 2 种：(v1) 内积 = 标量  /  (v2) 外积 = 秩1矩阵", fontsize=13
    )
    plt.show()
...
def mat_times_vec(A, x):
    ...
    ax = fig.add_subplot(gs1[2])
    _mat_ax(
        ax, result.reshape(-1, 1), "(Mv1) = Ax\n每行·x = 点积", color="#E76F51", **kw
    )
    ...
    for j in range(n):
        ...
        _mat_ax(ax, col, f"×{x[j]:.2g}\n(列{j})", color=_PAL[j % len(_PAL)], **kw)

    ax = fig.add_subplot(gs2[-1])
    _mat_ax(ax, result.reshape(-1, 1), "(Mv2) = Ax\n列的线性组合", color=INK, **kw)

    plt.suptitle("矩阵 × 向量 — 2 种：(Mv1) 行点积  /  (Mv2) 列的线性组合", fontsize=13)
    plt.show()
...
def mat_times_mat_rank1(A, B):
    ...
    for j in range(k):
        r1 = np.outer(A[:, j], B[j, :])
        _mat_ax(
            axes[0, j], r1, f"a_{j} ⊗ b_{j}ᵀ  (秩1)", color=_PAL[j % len(_PAL)], **kw
        )
        axes[1, j].bar(
            range(B.shape[1]), B[j, :], color=_PAL[j % len(_PAL)], alpha=0.75
        )
...
def show_factorization(A, factors, labels, modes=None, title=""):
    ...
    fig, axes = plt.subplots(1, len(all_mats), figsize=(3 * len(all_mats), 4))
