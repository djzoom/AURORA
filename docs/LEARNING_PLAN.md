...
## 3.5 数学前导课程（代码优先，足以支撑 Aurora 核心数学）

在啃 Audio Core 之前用 numpy 把数学地基打好。四门课全部在 `notebooks/prep_*/`，
代码优先、带 ✏️ 填空与自动判卷，每课都连到 Aurora 真实用途：
...
**推荐顺序**：先 `prep_complex_trig` + `prep_linear_algebra`（撑起 Audio Core）→ 做
Week 1 → 快进 Month 2 前再补 `prep_calculus` + `prep_probability`。这四块合起来
**足以解决 Aurora 普通工程领域的全部核心数学基础**。
