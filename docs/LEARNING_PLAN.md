...
| 课程 | 内容 |
|---|---|
| `0_foundation/L01_environment` | 环境配置、内核选择、依赖验证 |
| `0_foundation/L02_python_numpy` | Notebook 操作、变量/函数/数组最小动作 |
| `0_foundation/L03_course_map` | 全课程地图与面试能力对照 |

**① 数学地基**（四门，足以支撑 Aurora 全部核心数学）：

| 课程 | 内容 | 主要服务于 |
|---|---|---|
| `1_complex_trig/` L04–L08 | 正弦、复数、欧拉、傅里叶直觉 | DSP / FFT |
...
**推荐顺序**：先过 `0_foundation`（L01–L03，确认环境）→ `1_complex_trig` + `2_linear_algebra`（L04–L21）
→ 做 `5_audio_dsp` L32–L36 → 快进深度学习前再补 `3_calculus` + `4_probability`（L22–L31）。

## 4. Month 1 周计划（当前可执行）

逐日 checklist：`docs/week-01-checklist.md`（L32–L36）、`docs/week-02-checklist.md`（L37–L42）；
后续阶段 checklist 在对应学习开始前创建。
...
## 5. Month 2-6（课程 notebook 已全部建好）

对应课程在 `notebooks/month02/`–`notebooks/month06/`，逐日 checklist 到时再建。

- **L54–L65** (`notebooks/6_deep_learning/`)：从零 autograd（L54–L58）→ PyTorch 入门（L59–L61）→
  关键词识别分类器（L62–L64，用自己的 mel 特征）→ 训练可视化（L65）。
- **L66–L71** (`notebooks/7_asr/`)：CTC 原理 → Whisper 架构 → 云 GPU 微调 → WER 评估 → 可视化。
- **L72–L76** (`notebooks/8_music/`)：音乐特征 → embedding → 相似度搜索 → 推荐引擎 → 可视化。
- **L77–L82** (`notebooks/9_llm/`)：Transformer/Attention → LoRA → 本地 LLM 推理 → RAG（FAISS）→ Agent → 可视化。
- **L83–L85** (`notebooks/10_integration/`)：端到端流水线 → MLOps → Demo 打磨与面试材料。
