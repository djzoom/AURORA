...
- [ ] `jupyter lab` 启动，打开 `notebooks/week01/`
- [ ] 练习全部在 notebook 里做：每天一个 `dayN_*.ipynb`，✏️ 标记处填空
...
- [ ] 写：`day1_numpy.ipynb` — 用 numpy 生成 0~1 秒、16kHz 的时间轴 `t`
...
- [ ] 写：`day2_sine.ipynb` — 自己实现一个 `my_sine(freq, dur, sr)`
...
- [ ] 写：`day3_aliasing.ipynb` — 混叠对比图（notebook 内联显示）
