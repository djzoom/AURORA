...
- [ ] `make install` 跑通（editable 安装 + dev 依赖）
- [ ] `make test` 全绿（确认环境 OK）
- [ ] `pip install matplotlib jupyter`（画图 + notebook）
- [ ] 建目录 `scripts/learn/`，本周代码放这里
...
- [ ] 写：`scripts/learn/d1_numpy.py` — 用 numpy 生成 0~1 秒、16kHz 的时间轴 `t`
...
- [ ] 写：`scripts/learn/d2_sine.py` — 自己实现一个 `my_sine(freq, dur, sr)`
...
- [ ] 写：`scripts/learn/d3_aliasing.py` + 一张对比图存到 `scripts/learn/figs/`
...
- [ ] 写：`scripts/learn/d4_euler.py` — 画欧拉公式的可视化
- [ ] commit

## Day 5 — 读懂 io.py / windows.py，整周收口
...
- [ ] 更新 `ROADMAP.md` 的 Week-1 checkbox
- [ ] commit + `git push`
