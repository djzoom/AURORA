...
| 正文实际出现 | 293 |
| ✅ 已内联标注 | 292（**99.7%**，2026-07-13 批量补标 39 处后） |
| ⚠️ 残余 | 1（「频率 → Relative Frequency」词典键位误报，音频域的 频率（frequency）早已标注） |

> 术语词典已抽为 **[[terms]]**（图谱单一数据源，`build_knowledge_graph.py` 只读它）；
> [[annotation-audit]] 回归纯历史文档（初审缺口清单 + 补标记录），改动它不再影响图谱。
