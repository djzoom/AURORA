...
> **状态（2026-07-03）：未执行 / 规划中。** 本文档提出的「在 L93 中新增『云 GPU 实战』
> 章节」尚未落地——当前 `L93_mlops.ipynb` 不含该章节。请勿把本规划当作已完成内容引用；
> L93/L72 的正文以 notebook 实际内容为准。保留本文件作为未来扩写的蓝图。

> 纳入课程节点（计划）：在 L93（MLOps）中新增「云 GPU 实战」章节；
