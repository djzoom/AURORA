...
- [ ] 重读 L21 的 DFT 矩阵格：`W @ x` 与 `fft(x)` 对齐（5 分钟）
- [ ] 快速回顾 L35 旋转因子公式（3 分钟）
- [ ] 浏览 L36 附录 A「进入 L37 前：分治思维 5 分钟预习」
- [ ] 用一句话写下：`xw = signal * window` 为什么会成为 L37 的 DFT 输入
...
- [ ] 至少 5 个 commit、1 篇博客
- [ ] 读完 L42 附录「整段 FFT → STFT 多一维」
- [ ] 读完 L43「零件清点」+ 运行参数量化实测格
- [ ] L44 前先完成「单帧 STFT 半步练习」
