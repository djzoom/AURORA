...
| [L21.md](per_lesson/L21.md) | 矩阵即滤波 — DFT 矩阵 / Mel 矩阵：音频处理 = 矩阵乘法 | C+ |
| [L22.md](per_lesson/L22.md) | 导数（derivative）— 切线斜率、极限定义、数值微分 vs 解析微分 | C+ |
...
| [L25.md](per_lesson/L25.md) | 梯度下降 — 用一条直线拟合数据，从损失函数到权重更新公式 | C+ |
...
| [L39.md](per_lesson/L39.md) | 从零手写 FFT — Cooley-Tukey 递归实现，与 numpy.fft 误差 < 1e-10 | C+ |
| [L40.md](per_lesson/L40.md) | 频谱分析实战 — 幅度谱 / 相位谱 / 频率分辨率，440Hz+880Hz 混合信号 | C+ |
...
| [L58.md](per_lesson/L58.md) | 训练循环（training loop） — loss 计算、optimizer.step、收敛曲线，拟合 ma | C+ |
...
| [L65.md](per_lesson/L65.md) | 训练可视化 — Loss / Acc 曲线，梯度范数，权重分布直方图 | C+ |
...
| [L69.md](per_lesson/L69.md) | CTC 前向算法 — 所有路径概率求和，O(T×2L) 纯 NumPy 实现 | C |
| [L70.md](per_lesson/L70.md) | Whisper 架构解析 — Log-Mel 输入、Transformer Encoder-Decoder、多 | B- |
| [L71.md](per_lesson/L71.md) | Whisper 解码策略 — 贪婪解码与 Beam Search 从原理到实现 | C |
| [L72.md](per_lesson/L72.md) | Whisper 微调（fine-tuning） — LoRA 低秩注入 vs 全参数，中文 / 方言数据适配实 | C+ |
| [L73.md](per_lesson/L73.md) | WER 评估 — 词错误率（插入 / 删除 / 替换），jiwer 对比逐句分析 | B- |
| [L74.md](per_lesson/L74.md) | ASR 错误分析 — 替换/删除/插入模式，从 WER 到可改进方向 | B- |
| [L75.md](per_lesson/L75.md) | ASR 流水线图形化 — 波形 → 声谱图 → token → 文字路径可视化 | C+ |
| [L76.md](per_lesson/L76.md) | 音乐理论速成 — 音高、音程、色度（chroma）轮与十二平均律 | C+ |
| [L77.md](per_lesson/L77.md) | 音乐特征工程 — chroma、RMS 能量、ZCR，调用 aurora.music 从零实现 | B- |
| [L78.md](per_lesson/L78.md) | 节拍追踪从零实现 — onset 包络、自相关与 BPM 估计 | C+ |
...
| [L82.md](per_lesson/L82.md) | 音乐特征可视化 — 色度（chroma）图、节拍图、相似度热力图动态展示 | C+ |
...
| [L85.md](per_lesson/L85.md) | KV-Cache 从零实现 — 键值缓存矩阵更新，O(seq²)→O(seq) 加速 | C+ |
| [L86.md](per_lesson/L86.md) | 采样策略从零实现 — temperature / top-k / top-p，纯 NumPy | B- |
| [L87.md](per_lesson/L87.md) | 量化与本地推理 — INT8 量化原理，连接 HuggingFace 轻量推理 | C+ |
| [L88.md](per_lesson/L88.md) | TF-IDF 检索从零实现 — 词频逆文档频率，纯 NumPy 向量检索 | C+ |
| [L89.md](per_lesson/L89.md) | RAG 完整流水线 — 文档切片（Document Chunking）+ TF-IDF 检索 + 提示词（Pr | C+ |
| [L90.md](per_lesson/L90.md) | 对话式 RAG — 会话记忆（Conversation Memory）、来源归因与 Podcast Q&A 流 | C+ |
| [L91.md](per_lesson/L91.md) | 注意力图解 — 多头注意力权重热力图，LoRA 低秩结构可视化 | B- |
| [L92.md](per_lesson/L92.md) | 端到端流水线（pipeline） — 麦克风 → ASR → LLM → 文本回答，全链路组装 | C+ |
| [L93.md](per_lesson/L93.md) | MLOps 基础 — W&B 实验追踪、模型版本管理、Docker 打包与部署脚本 | C+ |
| [L94.md](per_lesson/L94.md) | Aurora v1 全景 Demo — 综合展示所有能力，面试材料与证据链整理 | B- |
| [L95.md](per_lesson/L95.md) | 研究论文入门——阅读、写作、投稿与学术合作 | B |
| [L96.md](per_lesson/L96.md) | 白板推导演练——FFT / CTC / 注意力机制（attention mechanism） | B+ |
| [L97.md](per_lesson/L97.md) | 面试准备与技术沟通——如何讲清你做的每一件事 | B- |
| [L98.md](per_lesson/L98.md) | 课程总结——做到了什么，还差什么 | C+ |
